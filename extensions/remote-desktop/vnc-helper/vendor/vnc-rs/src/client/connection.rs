    ...
    async fn input(&mut self, event: X11Event) -> Result<(), VncError> {
        if self.closed {
            ...
            let msg = match event {
                ...
                X11Event::CopyText(text) => ClientMsg::ClientCutText(text.into_bytes()),
                X11Event::CopyTextBytes(bytes) => ClientMsg::ClientCutText(bytes),
            };
    ...
    Fut: Future<Output = Result<(), VncError>>,
{
    ...
    while let Err(oneshot::error::TryRecvError::Empty) = stop_ch.try_recv() {
        ...
        match server_msg {
            ...
            ServerMsg::ServerCutText(bytes) => {
                output_func(VncEvent::CutTextBytes(bytes)).await?;
            }
        }
    }
    Ok(())
