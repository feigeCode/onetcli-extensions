...
pub enum VncEvent {
    ...
    ///
    Text(String),
    /// Raw ServerCutText bytes, preserved for caller-selected legacy decoding.
    CutTextBytes(Vec<u8>),
...
pub enum X11Event {
    ...
    ///
    CopyText(String),
    /// Send a pre-encoded ClientCutText payload without changing its bytes.
    CopyTextBytes(Vec<u8>),
}
