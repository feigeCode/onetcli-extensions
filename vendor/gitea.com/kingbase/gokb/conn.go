...
import (
	...
	"gitea.com/kingbase/gokb/oid"
	"gitea.com/kingbase/gokb/scram"
)
...
func network(o values) (string, string) {
	...
	if strings.HasPrefix(host, "/") {
		sockPath := path.Join(host, ".s.KBSQL."+o["port"])
...
func (cn *conn) auth(r *readBuf, o values) {
	...
	case 7: // GSSAPI, startup
		if newGss == nil {
			errorf("kerberos error: no GSSAPI provider registered (import gitea.com/kingbase/gokb/auth/kerberos if you need Kerberos support)")
		}
		cli, err := newGss()
		if err != nil {
...
//    tblname := "my_table"
//    data := "my_data"
//    quoted := gokb.QuoteIdentifier(tblname)
...
//
//    exp_date := gokb.QuoteLiteral("2023-01-05 15:00:00Z")
...
func parseEnviron(env []string) (out map[string]string) {
	...
	for _, v := range env {
		...
		// execution. Options which gokb expects to be set to a
